...
test('if validateWith works as expected', () => {
  ...
  expect(proof.validateWith(mockedValidator)).resolves.not.toThrow();

  mockedValidator.validate.mockReturnValueOnce(Promise.resolve(false));

  expect(proof.validateWith(mockedValidator)).rejects.toThrow(
...
test('if valueEquals works as expected', () => {
  ...
  expect(proof.valueEquals({} as AssetProof)).toBe(false);
  expect(
    proof.valueEquals(
      new AssetProof(
        '',
